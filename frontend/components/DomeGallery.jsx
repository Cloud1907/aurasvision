...
// Varsayılan görsel yok: bu kurulum yerel çalışır, üst bileşen kendi listesini verir.
...
export default function DomeGallery({
  ...
  // Kendiliğinden çok yavaş dönüş. Sürükleme, atalet ve büyütme sırasında durur;
  ...
  useEffect(() => {
    ...
    const step = now => {
      ...
      last = now;
      const bosta = !draggingRef.current && !inertiaRAF.current && !focusedElRef.current
        && rootRef.current?.getAttribute('data-enlarging') !== 'true';
      if (bosta) {
        rotationRef.current = {
          x: rotationRef.current.x,
          y: wrapAngleSigned(rotationRef.current.y + autoRotateDegPerSec * dt)
        };
        applyTransform(rotationRef.current.x, rotationRef.current.y);
      }
